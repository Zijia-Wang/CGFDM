    ...
    int FlagX, int FlagY, int FlagZ, int i0, int nfault)
{
  ...
  if (j < nj && k < nk ) { 
    ...
    if(par.freenode && km==1){
      ...
      for (ii = 0; ii < 3; ii++){
        for (jj = 0; jj < 3; jj++){
          matVx2Vz1    [ii][jj] = F.matVx2Vz1    [j1*9 + ii*3 + jj + matsize];      //wangzj
          ...
          matPlus2Min1f[ii][jj] = F.matPlus2Min1f[j1*9 + ii*3 + jj + matsize];
          matPlus2Min2f[ii][jj] = F.matPlus2Min2f[j1*9 + ii*3 + jj + matsize];
          matPlus2Min3f[ii][jj] = F.matPlus2Min3f[j1*9 + ii*3 + jj + matsize];
...
#ifdef DxV_T1f
      // =======================================================
      if(F.faultgrid[j+k*nj + mpifaultsize]){                 //wangzj
        dtT1[0] = F.hT11[j + k * nj + mpifaultsize];                   //wangzj
      ...
      if(F.faultgrid[j*nk+j + mpifaultsize]){
      pos = j1 + k1 * ny + (i0-1) * ny * nz;
      pos0 = j1 + k1 * ny + 0 * ny * nz;
    ...
    if(par.freenode && (km==2 || km==3) ){
      ...
      for (ii = 0; ii < 3; ii++){
        for (jj = 0; jj < 3; jj++){
          matPlus2Min1[ii][jj] = F.matPlus2Min1[idx + ii*3 + jj + matsize2];
          matPlus2Min2[ii][jj] = F.matPlus2Min2[idx + ii*3 + jj + matsize2];
          matPlus2Min3[ii][jj] = F.matPlus2Min3[idx + ii*3 + jj + matsize2];
          matPlus2Min4[ii][jj] = F.matPlus2Min4[idx + ii*3 + jj + matsize2];
          matPlus2Min5[ii][jj] = F.matPlus2Min5[idx + ii*3 + jj + matsize2];
...
#ifdef DxV_OSD
      // calculate backward on the minus side directly
      if(F.faultgrid[j*nk+j + mpifaultsize]){                 //wangzj
      pos = j1 + k1 * ny + (i0-1) * ny * nz;
      pos0 = j1 + k1 * ny + 0 * ny * nz;
      DxVx[3] = ( f_Vx[pos0] - w_Vx[pos] )*rDH;
      DxVy[3] = ( f_Vy[pos0] - w_Vy[pos] )*rDH;
    ...
    for (ii = 0; ii < 3; ii++){
      for (jj = 0; jj < 3; jj++){
        mat1[ii][jj] = F.D11_1[idx + ii*3 + jj + Dsize];       //wangzj
    ...
    int FlagX, int FlagY, int FlagZ, int i0, int nfault)
{
  ...
  dim3 grid(
      ...
      1);
  fault_dstrs_f_cu <<<grid, block>>> (W, F, M, FlagX, FlagY, FlagZ, i0, nfault);
  return;
}
