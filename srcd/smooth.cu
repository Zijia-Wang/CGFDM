...
__global__
void smooth_gauss_volume0(Wave w, Fault f, real_t *M, int i0, int nfault)
{
  ...
  int ny = nj + 6;
  int nz = nk + 6;
  int faultsize = nfault * ny*nz*9*2;
  int Tsize = nfault * ny*nz*7;
 // int i0 = nx/2;             *********wangzj
  int mpiFsize = nfault * nj*nk;
  // extend at free surface
  if(par.freenode && j < nj){
    int pos = j + k * nj + mpiFsize;
    if (f.smooth_flag[pos]){
    // f.W(ny, nz, 2, FSIZE)
      for (int l = 0; l < FSIZE; l++){
        for (int m = 0; m < 2; m++){
          for (int n = 1; n < SMOOTH_HALF_WIDTH1; n++){
            int pos_free  = l * ny * nz * 2 + m * ny * nz + (k1    ) * ny + j1 + faultsize;
            int pos_outer = l * ny * nz * 2 + m * ny * nz + (k1 + n) * ny + j1 + faultsize;
            int pos_inner = l * ny * nz * 2 + m * ny * nz + (k1 - n) * ny + j1 + faultsize;
...
__global__
void smooth_gauss_volume1(Wave w, Fault f, real_t *M, int i0, int nfault, int it)
{
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if(j < nj && k < nk){
    ...
    real_t gauss_width = 0.4;
    // if (nfault == 0){
    //   f.smooth_flag[pos] += 1;
    //   gauss_width = gauss_width - (f.smooth_flag[pos] - 1) * dt * 0.1 * 0.15;
    // } else if(dt * it >= 4){
    //   f.smooth_flag[pos] += 1;
    //   gauss_width = gauss_width - (f.smooth_flag[pos] - 1) * dt * 0.1 * 0.15;
    // }
    //   gauss_width = MAX(0.35, gauss_width);
    
    // if (f.smooth_flag[pos] == 2){
    //   gauss_width = 0.2;
    // }
    real_t pmax1 = 0.4;
    ...
    // smooth split nodes
      ...
      real_t *t21 = f.W + 3 * ny * nz * 2 + faultsize;
      real_t *t22 = f.W + 4 * ny * nz * 2 + faultsize;
      real_t *t23 = f.W + 5 * ny * nz * 2 + faultsize;
      real_t *t31 = f.W + 6 * ny * nz * 2 + faultsize;
      real_t *t32 = f.W + 7 * ny * nz * 2 + faultsize;
      real_t *t33 = f.W + 8 * ny * nz * 2 + faultsize;

      real_t *t21s = f.Ws + 3 * ny * nz * 2 + faultsize;
      real_t *t22s = f.Ws + 4 * ny * nz * 2 + faultsize;
      real_t *t23s = f.Ws + 5 * ny * nz * 2 + faultsize;
      real_t *t31s = f.Ws + 6 * ny * nz * 2 + faultsize;
      real_t *t32s = f.Ws + 7 * ny * nz * 2 + faultsize;
      real_t *t33s = f.Ws + 8 * ny * nz * 2 + faultsize;
...
__global__
void smooth_gauss_volume2(Wave w, Fault f, real_t *M, int nfault, int Faultgrid[])
{
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  int k = blockIdx.y * blockDim.y + threadIdx.y;
  int j1 = j + 3;
...
//       if (!f.united[k * nj + j + mpiFsize]){
//         real_t RS_L = 0.4;
//         real_t c = f.slip[k * nj + j + mpiFsize] / RS_L;
// #else
//       if (gj >= Faultgrid[0 + 4*nfault] +3 && gj <= Faultgrid[1 + 4*nfault] - 3 &&
//           gk >= Faultgrid[2 + 4*nfault] +3 && gk <= Faultgrid[3 + 4*nfault] ){
//       // if (gj >= par.Fault_grid[0] && gj <= par.Fault_grid[1] &&
//           // gk >= par.Fault_grid[2] && gk <= par.Fault_grid[3] ){
//         beta = 0.0;
//         // real_t c = f.slip[k * nj + j + mpiFsize] / (f.Dc[k * nj + j + mpiFsize]*1.0);      // wangzj *******
// #endif
//       }
      real_t c = 0.0;
      if (gj >= Faultgrid[0 + 4*nfault] && gj <= Faultgrid[1 + 4*nfault] &&
        ...
        if(f.init_t0_flag[pos]){
          if(par.Friction_type > 0){
            // real_t RS_L = f.L[pos];
            c = f.slip[pos] / f.L[pos];
          }
          else{
            c = f.slip[pos] / (f.Dc[pos]*1.0);
          }
        }
        beta = MIN(c, 1.0);
      }else{
      ...
      for (int l = 3; l < FSIZE; l++){
        for (int m = 0; m < 2; m++){
          int pos = l * ny * nz * 2 + m * ny * nz + k1 * ny + j1 + nfault * (ny*nz*2*9);
...
void smooth_gauss_volume(Wave w, Fault f, real_t *M, int i0, int nfault, int Faultgrid[], int it)
{
  dim3 block1(128, 1, 1);
  dim3 grid1(
  ...
  smooth_gauss_volume0 <<<grid, block>>> (w, f, M, i0, nfault);
  CUDACHECK(cudaGetLastError());
  smooth_gauss_volume1 <<<grid, block>>> (w, f, M, i0, nfault, it);
  CUDACHECK(cudaGetLastError());
  smooth_gauss_volume2 <<<grid, block>>> (w, f, M, nfault, Faultgrid);
  CUDACHECK(cudaGetLastError());
}
