...
__global__ void abs_deriv_x_cu(Wave w, real_t *M, PML P, int Flag, int idx0)
{
...
void abs_deriv_x(Wave W, real_t *M, PML P, int Flag, int idx0)
{
    dim3 block(16, 4, 4);
    dim3 grid(
        (hostParams.nj    + block.x-1)/block.x,
        (hostParams.nk    + block.y-1)/block.y,
        (hostParams.PML_N + block.z-1)/block.z);

  abs_deriv_x_cu <<<grid, block>>> (W, M, P, Flag, idx0);
  return;
}
